(* PostgreSQL-OCAML - OCaml-interface to the PostgreSQL database
   ...
   Copyright (C) 2004- Markus Mottl email: markus.mottl@gmail.com WWW:
   http://www.ocaml.info

   Copyright (C) 2001 Alain Frisch (version: postgres-20010808) email:
   Alain.Frisch@ens.fr WWW: http://www.eleves.ens.fr/home/frisch
...
external oid_of_ftype : ftype -> (oid[@untagged])
  = "oid_of_ftype_stub_bc" "oid_of_ftype_stub"
[@@noalloc]
...
type flush_status = Successful | Data_left_to_send
...
type put_copy_result = Put_copy_queued | Put_copy_not_queued | Put_copy_error
...
module Stub = struct
  ...
  external connection_status : connection -> connection_status = "PQstatus_stub"
  [@@noalloc]

  external error_message : connection -> string = "PQerrorMessage_stub"

  external backend_pid : connection -> (int[@untagged])
    = "PQbackendPID_stub_bc" "PQbackendPID_stub"
  [@@noalloc]

  external server_version : connection -> (int[@untagged])
    = "PQserverVersion_stub_bc" "PQserverVersion_stub"
  [@@noalloc]
  ...
  external exec_params :
    ...
    bool array ->
    bool ->
    result = "PQexecParams_stub_bc" "PQexecParams_stub"

  external prepare : connection -> string -> string -> oid array -> result
    = "PQprepare_stub"
  ...
  external describe_prepared : connection -> string -> result
    = "PQdescribePrepared_stub"

  external result_status : result -> result_status = "PQresultStatus_stub"
  [@@noalloc]

  external result_error : result -> string = "PQresultErrorMessage_stub"

  external result_error_field : result -> Error_field.t -> string
    = "PQresultErrorField_stub"

  external make_empty_res : connection -> result_status -> result
    = "PQmakeEmptyPGresult_stub"

  external ntuples : result -> (int[@untagged])
    = "PQntuples_stub_bc" "PQntuples_stub"
  [@@noalloc]
  ...
  external nfields : result -> (int[@untagged])
    = "PQnfields_stub_bc" "PQnfields_stub"
  [@@noalloc]
  ...
  external fmod : result -> (int[@untagged]) -> (int[@untagged])
    = "PQfmod_stub_bc" "PQfmod_stub"
  [@@noalloc]

  external fsize : result -> (int[@untagged]) -> (int[@untagged])
    = "PQfsize_stub_bc" "PQfsize_stub"
  [@@noalloc]
  ...
  external getisnull : result -> (int[@untagged]) -> (int[@untagged]) -> bool
    = "PQgetisnull_stub_bc" "PQgetisnull_stub"
  [@@noalloc]

  external getlength :
    result -> (int[@untagged]) -> (int[@untagged]) -> (int[@untagged])
    = "PQgetlength_stub_bc" "PQgetlength_stub"
  [@@noalloc]
  ...
  external oid_value : result -> (oid[@untagged])
    = "PQoidValue_stub_bc" "PQoidValue_stub"
  [@@noalloc]

  (* Asynchronous Query Processing *)

  external connect_poll : connection -> polling_status = "PQconnectPoll_stub"
  [@@noalloc]

  external reset_start : connection -> bool = "PQresetStart_stub" [@@noalloc]

  external reset_poll : connection -> polling_status = "PQresetPoll_stub"
  [@@noalloc]

  external set_nonblocking : connection -> bool -> (int[@untagged])
    = "PQsetnonblocking_stub_bc" "PQsetnonblocking_stub"
  [@@noalloc]

  external is_nonblocking : connection -> bool = "PQisnonblocking_stub"
  [@@noalloc]
  ...
  external send_prepare :
    ...
    = "PQsendPrepare_stub_bc" "PQsendPrepare_stub"
  [@@noalloc]
  ...
  external put_copy_data :
    ...
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "PQputCopyData_bc" "PQputCopyData_stub"

  external put_copy_end : connection -> string option -> (int[@untagged])
    = "PQputCopyEnd_bc" "PQputCopyEnd_stub"

  external get_copy_data : connection -> (int[@untagged]) -> get_copy_result
  ...
  external getline :
    ...
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "PQgetline_stub_bc" "PQgetline_stub"
  ...
  external getline_async :
    ...
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "PQgetlineAsync_stub_bc" "PQgetlineAsync_stub"
  [@@noalloc]
  ...
  external putnbytes :
    ...
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "PQputnbytes_stub_bc" "PQputnbytes_stub"
  ...
  external escape_string_conn :
    ...
    pos:(int[@untagged]) ->
    len:(int[@untagged]) ->
    string = "PQescapeStringConn_stub_bc" "PQescapeStringConn_stub"
  ...
  external escape_bytea_conn :
    ...
    pos:(int[@untagged]) ->
    len:(int[@untagged]) ->
    string = "PQescapeByteaConn_stub_bc" "PQescapeByteaConn_stub"

  (* Control Functions *)

  external set_notice_processor : connection -> (string -> unit) -> unit
    = "PQsetNoticeProcessor_stub"

  external set_notice_processor_num : connection -> int -> unit
    = "PQsetNoticeProcessor_num"
  ...
  external lo_open : connection -> (oid[@untagged]) -> (large_object[@untagged])
  ...
  external lo_tell : connection -> (large_object[@untagged]) -> (int[@untagged])
  ...
  external lo_read :
    connection ->
    (large_object[@untagged]) ->
    Bytes.t ->
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "lo_read_stub_bc" "lo_read_stub"
  ...
  external lo_read_ba :
    ...
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "lo_read_ba_stub_bc" "lo_read_ba_stub"
  ...
  external lo_write :
    ...
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "lo_write_stub_bc" "lo_write_stub"
  ...
  external lo_write_ba :
    ...
    (int[@untagged]) ->
    (int[@untagged]) ->
    (int[@untagged]) = "lo_write_ba_stub_bc" "lo_write_ba_stub"
  ...
  external lo_seek :
    ...
    (int[@untagged]) ->
    seek_cmd ->
    (int[@untagged]) = "lo_lseek_stub_bc" "lo_lseek_stub"
end
...
class result res =
  ...
  object
    ...
    method get_tuple t =
      check_tuple t;
      Array.init nfields (Stub.getvalue res t)
...
class connection ?host ?hostaddr ?port ?dbname ?user ?password ?options ?tty
        ...
        let field name = function
          ...
          | Some x ->
              ...
              for i = 0 to String.length x - 1 do
                if x.[i] = '\'' then Buffer.add_string b "\\'"
  ...
  fun () ->
     ...
     object (self (* Main routines *))
         ...
         let version =
           wrap_conn (fun conn ->
               ...
               if version <> 0 then version
                 ...
                 let msg =
                   if Stub.connection_status conn = Bad then
                     "server_version failed because the connection was bad"
       ...
       method exec ?(expect = []) ?(param_types = [||]) ?(params = [||])
           ?(binary_params = [||]) ?(binary_result = false) query =
         let r =
           wrap_conn (fun conn ->
               let r =
                 Stub.exec_params conn query param_types params binary_params
                   binary_result
               in
       ...
       method exec_prepared ?(expect = []) ?(params = [||])
           ?(binary_params = [||]) stm_name =
         let r =
           wrap_conn (fun conn ->
       ...
       method get_copy_data ?(async = false) () =
         wrap_conn (fun conn ->
             Stub.get_copy_data conn (if async then 1 else 0))
       ...
       method putline buf =
         wrap_conn (fun conn ->
             if Stub.putline conn buf <> 0 && not (Stub.is_nonblocking conn)
             then signal_error conn)
       ...
       method putnbytes ?(pos = 0) ?len buf =
         ...
         wrap_conn (fun conn ->
             if
               Stub.putnbytes conn buf pos len <> 0
               && not (Stub.is_nonblocking conn)
             then signal_error conn)
