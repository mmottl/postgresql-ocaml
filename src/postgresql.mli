...
type get_copy_result =
  ...
  | Get_copy_wait
      (** The next row is still being received (async only); wait for read-only,
          call [consume_input], and try again *)
...
(** Kinds of exceptions:
    ...
    [Binary] - result consists of binary tuple data [Connection_failure msg] -
...
(** [unescape_bytea str] unescapes binary string [str]. This function supports
    the new hex format for encoding bytea strings (introduced in Postgresql 9.0)
    even if the local libpq library is from an older version. *)
...
class type result = object
  ...
  (** [#nparams]
      ...
      @return
        the number of parameters of a prepared statement. This function is only
        useful when inspecting the result of [#describe_prepared]. For other
  ...
  (** [#paramtype n]
      ...
      @return
        the datatype of the indicated statement parameter. Parameter numbers
  ...
  (** [#paramtype n]
      ...
      @return
        the oid of the indicated statement parameter. Parameter numbers start at
        0. This function is only useful when inspecting the result of
        [#describe_prepared]. For other types of queries it will return zero.
  ...
  (** [#cmd_tuples]
      ...
      @return
        the object ID of the inserted row if the SQL command was an INSERT that
        inserted exactly one row into a table that has OIDs. Otherwise, returns
        [invalid_oid]. *)
...
(** Class of connections.
    ...
    When [conninfo] is given, it will be used instead of all other optional
    arguments.
...
object
  ...
  (** [#try_reset] tries to reset the connection if it is bad. If resetting
      fails, the [error] exception will be raised with [Connection_failure].
  ...
  (** [#set_notice_processor] controls reporting of notice and warning messages
  ...
  (** [exec ?expect ?params ?param_types ?binary_params ?binary_result query]
      synchronous execution of query or command [query]. The result status will
      be checked against all elements in [expect]. If [expect] is not empty and
      if there is no match, the exception [Unexpected_status] will be raised.
  ...
  method exec_prepared :
    ...
    string ->
    result
  (** [exec_prepared ?expect ?params ?binary_params stm_name] acts as [#exec],
  ...
  (** [getline ?pos ?len buf] reads a newline-terminated line of at most [len]
      characters into [buf] starting at position [pos].
  ...
  (** [getline_async ?pos ?len buf] reads a newline-terminated line of at most
      [len] characters into [buf] starting at position [pos] (asynchronously).
      No need to call [endcopy] after receiving [EndOfData].
  ...
  (** [putline str] sends [str] to backend server. Don't use this method for
      binary data, use putnbytes instead!

      @raise Error if there is a connection error. *)

  method putnbytes : ?pos:int -> ?len:int -> string -> unit
  (** [putnbytes ?pos ?len buf] sends the substring of [buf] of length [len]
      starting at [pos] to backend server (use this method for binary data).
  ...
  (** [copy_out_channel ch] sends each line returned by backend server to output
      channel [ch].

      @raise Error if there is a connection error. *)

  method copy_in_channel : in_channel -> unit
  (** [copy_in_channel ch] sends each line in input channel [ch] to backend
      server.
  ...
  (** [set_nonblocking b] sets state of the connection to nonblocking if [b] is
      true and to blocking otherwise.
  ...
  (** [request_cancel] requests that PostgreSQL abandon processing of the
      current command.
  ...
  (** [lo_import filename] imports an operating system file given by [filename]
      as a large object.

      @raise Error if there is a connection error. *)

  method lo_export : oid -> string -> unit
  (** [lo_export oid filename] exports the large object given by [oid] to an
      operating system file given by [filename].
      @raise Error if there is a connection error. *)
  ...
  (** [lo_write ?pos ?len buf lo] writes [len] bytes of buffer [buf] starting at
      position [pos] to large object [lo].
  ...
  (** [lo_read lo ?pos ?len buf] reads [len] bytes from large object [lo] to
      buffer [buf] starting at position [pos].
  ...
  (** [lo_seek ?pos ?whence lo] seeks read/write position [pos] in large object
      [lo] relative to the start, current read/write position, or end of the
      object ([whence] is SEEK_SET, SEEK_CUR, SEEK_END respectively).
